...
The application accepts only verified email addresses whose exact domain is `umd.edu`, `terpmail.umd.edu`, `umaryland.edu`, `rx.umaryland.edu`, or `rx.maryland.edu`. Those subdomains are explicit allow-list entries because the comparison is exact equality: every other campus subdomain, such as `dept.umd.edu`, `mail.umd.edu`, `mail.rx.umaryland.edu`, or `mail.rx.maryland.edu`, is still rejected. Google hosted-domain hints are not authorization controls.

Adding those named subdomains needs no separate Google Cloud or Supabase provider. The application never sends an `hd` hint, so the same OAuth client serves every exact domain.
...
3. Complete login with an exact `umd.edu`, `terpmail.umd.edu`, `umaryland.edu`, `rx.umaryland.edu`, or `rx.maryland.edu` account.
4. Confirm a new local profile goes to `/onboarding`; for `rx.umaryland.edu` and `rx.maryland.edu`, confirm `university_id` is `umb`. Repeat after onboarding and confirm it returns to `/app`.
